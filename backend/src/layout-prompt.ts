...
// The model acts as an art director: it INVENTS a unique layout + palette for
// this specific person. We constrain it to a safe vocabulary so the renderer
// can always draw it, but order/inclusion/columns/hero/colours/fonts are the
// model's own design decisions — different for every profile.
...
export function buildLayoutSystemPrompt(): string {
  return [
    ...
    "two different people should get visibly different pages.",
  ].join(" ");
}
...
export function buildLayoutUserPrompt(profile: GeneratedProfile): string {
  ...
  return [
    ...
    `Content available to arrange: ${available}. Has chat: ${profile.sections.includes("Chat")}. Booking: ${profile.booking.enabled}.`,
    "",
  ...
  "blocks": [ { "type": <one of: ${BLOCK_TYPES.join(", ")}>, "heading": "string", "columns": 1|2|3 } ]
    ...
    "Only include data blocks (experience/projects/services/skills/testimonials/faq) if that content exists above.",
    "Include a 'chat' block if chat is available, and a 'contact' block near the end.",
    "palette must be 6 valid hex colours with strong contrast between 'ink' and 'bg', and 'accentInk' readable on 'accent'.",
    "Give each block a short, human heading. ensure accessibility-level contrast.",
  ].join("\n");
}
...
/** Validate + clamp the model's spec so the renderer never receives junk. */
export function validateLayoutSpec(raw: unknown): LayoutSpec | undefined {
  ...
  const blocks: LayoutBlock[] = rawBlocks
    .map((b) => {
      ...
      // Drop duplicate sections (quote may repeat as a divider; everything else is unique).
