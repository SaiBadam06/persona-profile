...
export function buildExtractSystemPrompt(): string {
  return [
    ...
    "Normalize roles/titles cleanly. Summaries should be one tight factual sentence each.",
    "Infer the most recent role as the primary 'role'. Derive a concise 'headline' from the person's own words.",
  ].join(" ");
}
